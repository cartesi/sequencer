...
struct RunArgs {
    ...
    #[arg(long, env = "SEQ_WS_MAX_SUBSCRIBERS")]
    ws_max_subscribers: Option<usize>,
    #[arg(long, env = "SEQ_WS_MAX_CATCHUP_EVENTS")]
    ws_max_catchup_events: Option<u64>,
...
impl Config {
    fn from_run_args(args: RunArgs) -> Result<Self, String> {
        ...
        let config = Self {
            ...
            ws_max_subscribers: args
                .ws_max_subscribers
                .unwrap_or(defaults.ws_max_subscribers),
            ws_max_catchup_events: args
                .ws_max_catchup_events
                .unwrap_or(defaults.ws_max_catchup_events),
    ...
    fn validate(&self) -> Result<(), String> {
        ...
        if self.ws_max_subscribers == 0 {
            return Err("ws_max_subscribers must be > 0".to_string());
        }
        if self.ws_max_catchup_events == 0 {
            return Err("ws_max_catchup_events must be > 0".to_string());
        }
    ...
    fn effective(&self) -> EffectiveConfig {
        EffectiveConfig {
            ...
            ws_max_subscribers: self.ws_max_subscribers,
            ws_max_catchup_events: self.ws_max_catchup_events,
