...
use sequencer_core::application::{Deposit, Method, Transfer, Withdrawal};
use sequencer_core::user_op::{SignedUserOp, UserOp};
use sequencer_rust_client::{SequencerClient, SubmitRejected, SubmitTxError};
...
pub async fn run_ack_benchmark(config: AckRunConfig) -> BenchResult<AckRunReport> {
    ...
    let client = SequencerClient::new_with_timeout(config.endpoint.clone(), timeout)
        .map_err(|e| err(format!("invalid endpoint '{}': {e}", config.endpoint)))?;
...
pub async fn run_e2e_benchmark(config: E2eRunConfig) -> BenchResult<E2eRunReport> {
    ...
    let client = SequencerClient::new_with_timeout(config.endpoint.clone(), timeout)
        .map_err(|e| err(format!("invalid endpoint '{}': {e}", config.endpoint)))?;
    let ws_subscribe_url = config
        .ws_subscribe_url
        .clone()
        .map(|base| append_from_offset(base.as_str(), config.from_offset))
        .unwrap_or_else(|| client.ws_subscribe_url(config.from_offset));
    let domain = default_domain();
    let mut workload = WorkloadState::initialize(
        &config.workload,
        config.seed_offset,
    ...
    let mut ws = if config.ws_subscribe_url.is_some() {
        connect_async(ws_subscribe_url.as_str())
            .await
            .map(|(stream, _)| stream)
            .map_err(|e| {
                io_err(format!(
        ...
        client.subscribe(config.from_offset).await.map_err(|e| {
            io_err(format!(
                "ws connect failed: url={ws_subscribe_url}, error={e}"
            ))
        })?
...
pub async fn submit_tx(
    ...
    let client = SequencerClient::new_with_timeout(endpoint.to_string(), timeout)
        .map_err(|e| err(format!("invalid endpoint '{endpoint}': {e}")))?;
    match client.submit_tx(req).await {
        Ok(response) => Ok(response),
        Err(SubmitRejected::Transport(transport_err)) => {
            Err(err(format!("tx submit failed: {transport_err}")))
        }
        Err(SubmitRejected::Http { status, body }) => Err(err(format!(
            "/tx rejected with status {status}: {body}. Hint: sequencer frame fee and payload-size bounds must allow these txs."
        ))),
        Err(SubmitRejected::Decode(reason)) => {
            Err(err(format!("tx submit decode failed: {reason}")))
        }
    }
}
...
async fn setup_funded_accounts(
    ...
    for account in accounts {
        ...
        let (status, body) = client
            .submit_tx_with_status(&fixture.request)
            .await
            .map_err(|e| {
                err(format!(
...
fn append_from_offset(base_ws_subscribe_url: &str, from_offset: u64) -> String {
    let separator = if base_ws_subscribe_url.contains('?') {
        '&'
    } else {
        '?'
    };
    format!("{base_ws_subscribe_url}{separator}from_offset={from_offset}")
}
